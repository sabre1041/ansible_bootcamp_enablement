...
There are several ways to monitor what actions the operator are taking:

* You can track the logs of the `<component<>-controller-operator-manager-<id>` pod logs.
...
When the `ansibleautomationplatform` CRD is successfully deployed then status of it and the `automationcontroller` instance should show `Conditions: Running, Successful`.
...
Go ahead and attach a subscription to this deployment by using a service account and password and choosing any valid subscription. Your Red Hat login used for cloud.redhat.com should have many subsctions to choose from.
...
By default, the container group is setup to deploy job pods in the same namesapce that the `AnsibleAutomationPlatform` CRD was created in.
...
Now, let's walk thru making a slight change the default container group on the deployed AAP instance.
...
Let's modify the spec slighty to add a custom label to each job pod.
