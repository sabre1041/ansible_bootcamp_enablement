...
* Create and configure Tekton pipelines on OpenShift
...
. In the Dev Spaces Workspace, create a new file called *collection-pipeline.yml* in the _ansible_bootcamp_ci_cd_pipeline_ directory, that we'll later push to the _ansible_bootcamp_ci_cd_pipeline_ repository

. Open file _collection-pipeline.yml_ in the Dev Spaces Workspace, and copy the contents *collection-pipeline.yml* from below into the _collection-pipeline.yml_ file
...
[start=3]
. In the Dev Spaces Workspace, create a new file called *collection-cluster-trigger-binding.yml* in the _ansible_bootcamp_ci_cd_pipeline_ directory, that we'll later push to the _ansible_bootcamp_ci_cd_pipeline_ repository

. Open file _collection-cluster-trigger-binding.yml_ in the Dev Spaces Workspace, and copy the contents *collection-cluster-trigger-binding.yml* from below into the _collection-cluster-trigger-binding.yml_ file
...
. In the terminal of your Dev Spaces Workspace, change into the _ansible_bootcamp_ci_cd_pipeline_ directory
...
Now that you have implemented automated CI/CD for Ansible collections, consider these advanced topics:

* **xref:07-configuration-as-code.adoc[Configuration as Code]**: Learn how to manage your entire AAP configuration as code
* **Advanced Pipeline Features**: Explore pipeline parameterization, conditional execution, and multi-stage deployments
* **Integration Testing**: Add integration tests that validate collections against real AAP instances
* **Security Scanning**: Implement automated security scanning in your pipelines
* **Monitoring & Observability**: Set up comprehensive logging and monitoring for your CI/CD workflows
