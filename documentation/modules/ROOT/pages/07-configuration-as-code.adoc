...
With Configuration as Code mastered, you're ready for advanced AAP deployment scenarios:

* **xref:08-aap-openshift.adoc[AAP on OpenShift]**: Learn operator-based AAP deployment on OpenShift
* **Multi-Environment Deployments**: Extend your config-as-code to manage development, staging, and production environments
* **GitOps Integration**: Implement GitOps workflows for continuous configuration updates
* **Backup and Recovery**: Create automated backup and disaster recovery procedures
