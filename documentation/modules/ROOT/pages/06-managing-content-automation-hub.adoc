...
. On the API Token page, click the 'Generate token' button.
...
[source,python,title="ansible_bootcamp/my_collection/plugins/filter/cowsay_filter.py"]
----
# ansible_bootcamp/my_collection/plugins/filter/cowsay_filter.py
...
First, edit `ansible_bootcamp/my_collection/galaxy.yml` and change the version from `1.0.0` to `1.0.1`.
